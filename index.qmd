---
title: "Analysis of Suicide and Unemployment Data in 2019"
...
complete_plot_10k <- 
  complete_data %>% 
  filter(year > 2005) %>% 
  filter(country %in% c("Lesotho", "Ukraine", "South Korea", "Guyana", "Suriname", "UAE")) %>% 
 ggplot(aes(x = suicides, 
            y = hdi, 
            frame = year,
            color = country)) +
...
complete_top_ggplotly <- ggplotly(complete_plot_10k)

complete_10K_ggplotly <- ggplotly(complete_plot_top)
```
